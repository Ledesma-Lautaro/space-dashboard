...
- **D-12:** All panels share a common terminal chrome: a `Panel` component with a title bar and ASCII borders (e.g. `+-- NEO FEED ------+`). **AMENDED after Phase 1 research:** the original wording specified Unicode box-drawing characters, but VT323 ships no glyphs in `U+2500─257F` (nor `■` at `U+25A0`) — verified against the served Google Fonts `unicode-range` declarations — so those characters would fall back to another font at a different advance width and break the very alignment the border mechanism exists to protect. User approved substituting plain ASCII (`+`, `-`, `|`, `#`), which VT323 covers at an identical advance. The decision (shared terminal chrome, character-grid alignment) is unchanged; only the glyph set is. Every later phase reuses it rather than inventing panel markup. — **Reversibility:** costly — Phases 4, 5 and 10 all build inside this component; changing its structure later means touching each of them.
...
**Known tension worth planning around (partly RESOLVED by Phase 1 research):** ASCII borders (D-12) and fluid responsive layout fight each other. Two measured corrections landed in `01-UI-SPEC.md`: the glyph set is now plain ASCII (VT323 has no box-drawing coverage), and VT323’s real advance is `0.4em` not `0.6em`, which supersedes the original 32/48/64ch tier scale — the executor measures the installed font and sets tiers so the compact tier fills a real phone width. Box characters misalign when a container's width is not an exact multiple of the character advance. This is solvable — size panel containers in `ch` units, and/or degrade the ASCII frame to a plain CSS border below a threshold width — but the planner should treat it as a real risk with an explicit mitigation task, not as an incidental styling detail. It is the most likely source of rework in this phase.
